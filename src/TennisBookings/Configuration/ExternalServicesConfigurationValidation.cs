...
public class ExternalServicesConfigurationValidation : IValidateOptions<ExternalServicesConfiguration>
{
    ...
    public static ValidateOptionsResult ValidateWeatherApiConfig(
		bool weatherForecastEnabled, ExternalServicesConfiguration options)
    {
        if (weatherForecastEnabled && string.IsNullOrEmpty(options.Url))
        {
            return ValidateOptionsResult
				       .Fail("A URL for the weather API is required when weather forecasting is enabled.");
        }
        if (options.MinsToCache < 10)
        {
            return ValidateOptionsResult
				     .Fail("The weather forecast cachine must be at least 10 minutes.");
        }
