...
public class ProductsApiClient : IProductsApiClient
{
		...
		ILogger<ProductsApiClient> logger, IMemoryCache cache)
    {
		...
		var externalServicesConfiguration =
		   options.Get(ExternalServicesConfiguration.ProductsApi);
    ...
    public async Task<ProductsApiResult?> GetProducts()
    {
        ...
        try
        {
            ...
            if (content != null)
            {
                ...
                {
                    AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(_productsApiConfig
					     .Get(ExternalServicesConfiguration.ProductsApi).MinsToCache)
				};
